...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		String name ="Nume";
		studentGradesMap.put(name, 9.5);
		double grade=studentGradesMap.get(name);
		studentGradesMap.remove(name);
		Set<String>keySet=studentGradesMap.keySet();
		Collection<Double>valueSet=studentGradesMap.values();
		Set<Map.Entry<String, Double>>entrySet=studentGradesMap.entrySet();
		for(Map.Entry<String, Double>entry:entrySet) 
		{
			String inName=entry.getKey();
			Double inValue=entry.getValue();
	...
	{
		double[][] autoscaledLearningSet = new double[learningSet.length][learningSet[0].length-1];
		//.. your code here
